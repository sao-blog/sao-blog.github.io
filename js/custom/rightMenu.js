...
function switchReadMode () { // read-mode
  ...
  if ($body.classList.contains('read-mode')) {
    ...
    document.querySelector('#menu-readmode>span').innerHTML = '閱讀模式'
    btf.snackbarShow('已關閉閱讀模式')
    return
  }
  $body.classList.add('read-mode')
  document.querySelector('#menu-readmode>span').innerHTML = '退出閱讀模式'
  btf.snackbarShow('已開啟閱讀模式')
...
function hideAsideBtn () { // Hide aside
  ...
  if ($htmlDom.contains('hide-aside')) {
    ...
    document.querySelector('#menu-hideside>span').innerHTML = '隱藏側欄'
    btf.snackbarShow('已顯示側欄')
  } else {
    saveToLocal.set('aside-status', 'hide', 2)
    document.querySelector('#menu-hideside>span').innerHTML = '顯示側欄'
    btf.snackbarShow('已隱藏側欄')
  }
...
window.oncontextmenu = function (event) {
  ...
  if (!saveToLocal.get('notice-rightmenu')) {
    btf.snackbarShow('喚醒原系統菜單請使用：<kbd>Ctrl</kbd> + <kbd>右鍵</kbd>', false, 5000)
...
document.getElementById('menu-baidu').onclick = function () { window.open('https://www.google.com.tw/search?q=' + rightMenuContext.text) }
